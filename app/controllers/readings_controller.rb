class ReadingsController < ApplicationController
  def reading
    ...
    @terms.each do |term|
      @article.gsub!(/\b#{Regexp.escape(term.english)}\b/) { "<span class='chinese' data-toggle='popover' data-trigger='hover' data-placement='auto' data-content='#{term.english}'>#{term.chinese_traditional}</span>" }
    end
  end
end
